...
| State management | BLoC | One bloc per domain — `AuthBloc`, `ArtworkBloc` |
...
UI (screens / widgets)
  ↓  events
BLoC          AuthBloc · ArtworkBloc · (planned) CollectionBloc
  ↓  calls
Repository    ArtRepository (AIC API) · AuthRepository (Firebase) · (planned) CollectionRepository (Firestore)
  ↓
...
- [ ] `CollectionRepository` + `CollectionBloc`
- [ ] Browse screen
